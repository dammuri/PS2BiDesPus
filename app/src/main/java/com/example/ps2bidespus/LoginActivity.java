...
public class LoginActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        sharedSession isSession = new sharedSession(LoginActivity.this);
        int idUsr = isSession.getSession();
        if(idUsr != -1)
        {
            Intent hommie = new Intent(LoginActivity.this,Home.class);
            startActivity(hommie);
            finish();
        }
        else {
                    ...
                    mDatabase.child("login").addValueEventListener(new ValueEventListener() {
                        ...
                        public void onDataChange(@NonNull DataSnapshot snapshot) {
                            ...
                            if (usr.equals(adminUsr) && pwd.equals(adminPass)) {
                                User user = new User(adminUsr, 1);
                                sharedSession simpan = new sharedSession(LoginActivity.this);
                                simpan.saveSession(user);
